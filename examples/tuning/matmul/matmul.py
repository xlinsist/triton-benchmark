...
GROUP_SIZE_M = 8
CACHE_PADDING = os.getenv("CACHE_PADDING", "0") != "0" # no padding
PREPACKED = os.getenv("PREPACKED", "1") != "0" # no prepacked
PAD_B_ONLY = True
...
def get_matmul_kernel_autotune_config():
    return [triton.Config({'BLOCK_SIZE_M': 8, 'BLOCK_SIZE_N': 32, 'BLOCK_SIZE_K': 8, 'GROUP_SIZE_M': 8}, num_threads=1),
            triton.Config({'BLOCK_SIZE_M': 32, 'BLOCK_SIZE_N': 32, 'BLOCK_SIZE_K': 32, 'GROUP_SIZE_M': 8}, num_threads=1),]
    # configs=[]
    # for GROUP_SIZE_M in [1, 4, 8, 16]:
    #     for BLOCK_SIZE_M in [4, 8, 16, 32]:
    #         for BLOCK_SIZE_N in [4, 8, 16, 32]:
    #             for BLOCK_SIZE_K in [4, 8, 16, 32]:
    #                 configs.append(triton.Config({'BLOCK_SIZE_M': BLOCK_SIZE_M, 'BLOCK_SIZE_N': BLOCK_SIZE_N, 'BLOCK_SIZE_K': BLOCK_SIZE_K, 'GROUP_SIZE_M': GROUP_SIZE_M}, num_threads=1))
    return configs
@triton.autotune(
...
def get_matmul_kernel_autotune_config():
    ...
    for GROUP_SIZE_M in [1, 4, 8, 16]:
        for BLOCK_SIZE_M in [4, 8, 16, 32]:
            for BLOCK_SIZE_N in [4, 8, 16, 32]:
                for BLOCK_SIZE_K in [4, 8, 16, 32]:
                    configs.append(triton.Config({'BLOCK_SIZE_M': BLOCK_SIZE_M, 'BLOCK_SIZE_N': BLOCK_SIZE_N, 'BLOCK_SIZE_K': BLOCK_SIZE_K, 'GROUP_SIZE_M': GROUP_SIZE_M}, num_threads=8))
    return configs
@triton.autotune(
    configs=get_matmul_kernel_autotune_config(),
    key=['M', 'N', 'K'],
...
def matmul_preprocess_input(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, num_threads=8):
    # Check constraints.
...
def matmul(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, M: int, N: int, K: int, num_threads=8):
    if not PREPACKED:
...
triton_output = matmul(a, b, c, 512, 512, 512, num_threads=1)
