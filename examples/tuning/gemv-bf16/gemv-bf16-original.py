...
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_SIZE_M': 8, 'BLOCK_SIZE_N': 64}, num_threads=1),
        # triton.Config({'BLOCK_SIZE_M': 8, 'BLOCK_SIZE_N': 8}, num_threads=1),
        # triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 8}, num_threads=1),
...
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_SIZE_M': 8, 'BLOCK_SIZE_N': 64}, num_threads=16),
        # triton.Config({'BLOCK_SIZE_M': 8, 'BLOCK_SIZE_N': 8}, num_threads=16),
        # triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 8}, num_threads=16),
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        x_vals=[(16 * i, 64) for i in range(1, 1500, 128)],  # Different possible values for `x_name`
